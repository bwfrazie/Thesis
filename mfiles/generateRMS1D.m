...
for Lcounter = 1:length(L)
    dispstring = sprintf('Processing L = %d m, number %d of %d',L, Lcounter,length(L));
    disp(dispstring);
    for(alphaCounter = 1:length(alpha))
        N = alpha(alphaCounter)*L(Lcounter);
        dispstring = sprintf('Processing N = %dL, number %d of %d',alpha, alphaCounter,length(alpha));
        disp(dispstring);
...
%                 dispstring = sprintf('Current RMS: %0.5f m, Total RMS: %0.5f m',std(h), totalRMS);
%                 disp(dispstring);
            end
