...
Here $L_0$ represents the deterministic component due to reflection from the surface and $L_s$ represents the random component due to reflection from the surface. The primary reflection point, $x_m$, should be a saddle point and provide the dominant contribution. We can therefore perform a Taylor expansion of $L_0$ about $x_m$.
...
&=\exp\left[-jk_oL_{so}\right]\exp\left[-jk_o\left(\frac{L_0''}{2}(x-x_m)^2-L_s\right)\right]\\
&=\exp\left[-jk_oL_{so}\right]\exp\left[\frac{-jk_oL_0''}{2}(x-x_m)^2\right]\exp\left[jk_oL_s \right]\\
\label{gf_eq:67}
\end{aligned}
\end{equation}
\renewcommand{\baselinestretch}{2} \small\normalsize

Because $L_{so}$ is not dependent on $x$, it can come outside of the integral. If we assume that $L_s$ is locally sinusoidal with amplitude $\sigma$, frequency $k_{\omega}$, and phase $\theta$, we can approximate the exponential term as 

\begin{equation}
\begin{aligned}
\exp\left[jk_oL_s\right] =\exp\left[j\sigma \cos\left(k_{\omega} (x-x_m) + \theta\right) \right]
\end{aligned}
\label{gf_eq:68}
\end{equation}
\renewcommand{\baselinestretch}{2} \small\normalsize
...
\begin{equation}
\begin{aligned}
& \exp\left[j\sigma \cos\left(k_{\omega} (x-x_m) + \theta\right) \right] \\ &=\sum_{l=-\infty}^{\infty}J_l(\sigma)\exp\left[jl(k_{\omega}(x-x_m) + \theta) \right] \\
...
\begin{gathered}
\exp\left[-jk_o\left( L_2 + L_3\right) \right]= \\
\exp\left[-jk_oL_{so}\right]\sum_{l=-\infty}^{\infty}J_l(\sigma)\exp\left[jl\theta\right]
...
\begin{equation}
\begin{gathered}
U_2(P_2) = \frac{1}{4\pi} \exp[-jk_oL_{so}] \sum_{l=-\infty}^{\infty}J_l(\sigma)\exp\left[j\phi_l\right] \\
...
\begin{equation}
\boxed{U_2(P_2) = \sqrt{\frac{1}{8\pi jk_o L}}\Gamma_1e^{-jk_oL_{so}} \sum_{l=-\infty}^{\infty}J_l(\sigma)e^{j\phi_l}} 
\label{gf_eq:73}
\end{equation}
\renewcommand{\baselinestretch}{2} \small\normalsize

A simple plane wave solution would result in $U_2(P_2) = \Gamma_1\exp[-jk_oL_{so}]$ for the reflected wave. Here, we see that the deterministic component from the paraxial wave  solution scales the plane wave solution by the geometric spreading of a paraxial wave. The random sea height contributions are now contained in the sum of Bessel functions.

\subsection {Knife Edges and Non-planar Terrain}
In many cases of interest, the surface we want to propagate over is not planar. In this case, we can formulate the diffraction integral in altitude (perpendicular to the surface) and treat subsequent obstacles as knife edges \cite{whitteker_diffraction}. If we treat the knife edge as an aperture that starts at $z=\xi$, assume no $y$ dependence from the source, $U(x,y) = U(x)$, and let propagation be along the $x$ axis, we can integrate out the $y$ dependence from the Fresnel diffraction equation (Equation \ref{gf_eq:53d}) and rewrite it as 

\begin{equation}
\begin{aligned}
U(P_2) &=\frac{je^{-jk_ox}}{\lambda x}\int_{\xi}^{\infty}\int_{-\infty}^{\infty} U(z') \exp\left[-j \frac{k}{2x}\left(z-z'\right)^2 \right]   \exp\left[-j \frac{k}{2x}\left(y-y'\right)^2 \right]dz' dy' \\
&=\frac{je^{-jk_ox}}{\lambda x}\sqrt{\frac{\pi 2 x}{jk_o}}\int_{\xi}^{\infty}U(z') \exp\left[-j \frac{k}{2x}\left(z-z'\right)^2 \right]dz'\\
\end{aligned}
\label{gf_eq:82}
\end{equation}
\renewcommand{\baselinestretch}{2} \small\normalsize
...
\begin{equation}
\begin{aligned}
\boxed{U(P_2) =\sqrt{\frac{jk_o}{2\pi x}}\int_{\xi}^{\infty}U(z') \exp\left[-j \frac{k}{2x}\left(z-z'\right)^2 \right]dz'}\\
\end{aligned}
\label{gf_eq:82a}
\end{equation}
\renewcommand{\baselinestretch}{2} \small\normalsize
...
The analytical results previously given have all assumed propagation in Cartesian coordinates. This section derives the parabolic wave equation (PWE) in spherical coordinates and presents a numerical implementation for solution through a split-step Fourier beam propagation method. 

\subsubsection{Analytical Derivation}
Because the earth is actually curved and not flat, it is often useful to treat the earth as a sphere when analyzing propagation. If we let $a_e$ be the radius of the earth, we can use a conformal mapping to implement the transformation \cite{kuttler_pe_theory}.
...
The only difference between Equation \label{gf_eq:75} and the previously derived paraxial wave equation is the inclusion of the term $2z/a_e$ that represents the impact of the curvature of the Earth.

There are two comments regarding this derivation. First, we have used the convention of spatial dependence going as $e^{-jk_ox}$, which leads to the $-$ sign for the $\partial A/\partial x$ term in Equation \ref{gf_eq:75}. If we had instead assumed the spatial dependence went as $e^{+jk_ox}$, we would have a $+$ sign for the $\partial A/\partial x$ term. Second, as previously mentioned this equation only defines how the amplitude changes with propagation and we will need to multiply the result by a plane wave, $e^{-jk_ox}$, to get the final answer. 

To solve the PWE we make a substitution for the modified refractivity, $m = n^2-1 + 2z/a_e$, and treat it as a constant. 
...
\noindent We can start by taking the Fourier transform of Equation \ref{gf_eq:76} along $z$
...
We can convert Equation \ref{gf_eq:79} into a marching algorithm by letting $x\rightarrow \Delta x = x-x_o$.
...
Equation \ref{gf_eq:80} is the split step algorithm for solving the PWE. One note of caution should be made here is to make sure the numerical Fast Fourier Transform (FFT) implementations match the convention used for forward and reverse transforms. If they do not agree, we would need to multiply the arguments of the exponential terms by $-1$ in Equation \ref{gf_eq:80}.

\begin{equation}
A(x,z) = \exp\left[j\frac{k_om}{2}\Delta x\right]\mathcal{F}^{-1}\left\{\exp\left[-j\frac{k_z^2}{2k_o}\Delta x \right]\hat{A}(x_o,k_z) \right\}
\label{gf_eq:81}
\end{equation}
\renewcommand{\baselinestretch}{2} \small\normalsize
