...
\begin{equation}
\boxed{U_2(P_2) = \sqrt{\frac{1}{8\pi jk_o L}}\Gamma_1e^{-jk_oL_m} \sum_{l=-\infty}^{\infty}J_l(\sigma)e^{j\phi_l}} 
\label{gf_eq:73}
\end{equation}
\renewcommand{\baselinestretch}{2} \small\normalsize
...
The only difference between Equation \ref{gf_eq:75} and the previously derived paraxial wave equation is the inclusion of the term $2z/a_e$ that represents the impact of the curvature of the Earth.
...
We can make a substitution for the modified refractivity, $m = n^2-1 + 2z/a_e$, and treat it as a constant. 
...
Equation \ref{gf_eq:80} is the split step algorithm for solving the PWE. One note of caution here is that when using numerical Fast Fourier Transform (FFT) implementations, we need to ensure they match the conventions used for forward and reverse transforms. If they do not agree, we would need to use the complex conjugates of the exponential terms in Equation \ref{gf_eq:80}.
