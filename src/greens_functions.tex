...
If we had flipped the order of the scalar variables and instead let $\mathbf{C}=\psi\nabla\varphi$, Green's first identity would be expressed as:
\begin{equation}
...
Equation \ref{gf_eq:9} shows that the Green's function is the solution to the differential equation for a spatial impulse and therefore represents the impulse response of the system. In electromagnetics, the driving force often has a negative sign as seen in the Laplace equation, $\nabla^2\varphi = -\rho/\epsilon_o$, so we typically chose a matching sign for the delta function as shown in Equation \ref{gf_eq:9}. With this convention, we need to make sure the driving force has a negative sign when utilizing a Green's function found in this manner.
...
\int\limits_{V}d^3r'\varphi\delta\left(\mathbf{r}-\mathbf{r}' \right) = \oint\limits_{S}\left[G\frac{\partial \varphi}{\partial n} - \varphi\frac{\partial G}{\partial n} \right]dS +\int\limits_{V}d^3r'\left[ Gk^2\varphi - \varphi k^2G-k^2G - Gf \right] \\
...
\begin{equation}
\boxed{\varphi = \left<\varphi \right> + \oint\limits_{S}dSG\frac{\partial \varphi}{\partial n}  -\int\limits_{V}d^3r' Gf}
\label{gf_eq:18}
\end{equation}
\renewcommand{\baselinestretch}{2} \small\normalsize
...
\begin{equation}
\mathbf{C}\left(\mathbf{r},t\right) = \mathbf{C}e^{j\left(\omega t - \mathbf{k} \cdot \mathbf{r} \right)}
\label{gf_eq:18b}
\end{equation}
\renewcommand{\baselinestretch}{2} \small\normalsize
...
To help visualize this Green's function, the magnitude and phase is shown in Figure \ref{gf_fig:1} and the real and imaginary components are shown in Figure \ref{gf_fig:2}. These figures constrain the Green's function to the $x-y$ plane ar $z=0$ for visualization.
...
\noindent Equation \ref{gf_eq:40c} is valid for $z>0$ . Since $k| \boldsymbol{\rho} - \boldsymbol{\rho}'| > 0$, this condition is satisfied. Letting $\rho' \rightarrow 0$ for simplification yields:
...
&= \frac{1}{2jk_o}\frac{1}{2\pi} \sqrt{\frac{\pi 2k_o}{jx}}e^{-j\frac{k_o}{2x}z^2 } \\
...
\section {Tabularized Green's Functions}
...
This section describes how to use Green's functions to find solutions for propagation problems.

\subsection {Free Space Propagation}
For free space propagation problems, we only have the forcing function of Equation \ref{gf_eq:11} surviving so that $U = -\int d^3r' Gf$.

\subsubsection{Point Source}
The simplest implementation is to assume a point source, so that $f= -\alpha \delta(\mathbf{r}-\mathbf{r}')\delta(t-t')$, in which case the solution simply becomes $U = G(\mathbf{r},\mathbf{r}',t,t')$.

The point source approximation is adequate to capture propagation characteristics in many cases, especially at long ranges. It is only at closer ranges that the distributed aspects of a source become apparent, and where the infrared signature of an object may look dramatically different from a point source.
...
Following Huygen's principle, we can use every point on the aperture as a source for the reflected wave. In order to ensure the integral over the surface vanishes at far distances, we need to apply the Sommerfeld radiation condition on the disturbance $U$
...
We will assume there is no additional forcing function in the aperture so the solution from Equation \ref{gf_eq:11} becomes $U = \oint dS\left[G\partial U/\partial n - U\partial G/\partial n  \right]$.

The Rayleigh-Sommerfeld approach is to can choose an alternative Green's function that is generated by a pair of mirrored point sources. This configuration will ensure that the Green's function vanishes on the surface so we do not need to apply boundary conditions to both $U$ and $\partial U/\partial n$. This Greens function, $G\_$, is then given as
...
With $\theta_1$ equal to the angle between $\hat{n}$ and $r_1$ and $\theta_2$ equal to the angle between $\hat{n}$ and $r_2$, the normal derivative, $\partial G\_/\partial n$, is then given as 

\begin{equation}
\begin{aligned}
\frac{\partial G\_}{\partial n} = \cos(\theta_1)\left(-jk_o - \frac{1}{r_1} \right)\frac{\exp[-jk_or_1]}{4\pi r_1} -\cos(\theta_2)\left(-jk_o - \frac{1}{r_2} \right)\frac{\exp[-jk_or_1]}{4\pi r_2}
\end{aligned}
\label{gf_eq:50}
\end{equation}
\renewcommand{\baselinestretch}{2} \small\normalsize

\noindent In the aperture, $\cos(\theta_1) = -\cos(\theta_2)$ and $r_1=r_2$ so that

\begin{equation}
\begin{gathered}
\frac{\partial G\_}{\partial n}\bigg|_S = -2\frac{\partial G}{\partial n}\bigg|_S \\
\end{gathered}
\label{gf_eq:51}
\end{equation}
\renewcommand{\baselinestretch}{2} \small\normalsize
...
\begin{equation}
\begin{gathered}
U(P_2) = -\oint\limits_{S}U_2(P_1)\frac{\partial G\_}{\partial n}dS\\
= 2\oint\limits_{S}U(P_1)\frac{\partial G}{\partial n}dS
\end{gathered}
\label{gf_eq:52}
\end{equation}
\renewcommand{\baselinestretch}{2} \small\normalsize
...
\subsubsection {Integration Along the Surface}

\subsubsection {Integration Perpendicular to the Surface}
