...
Multiplying by $e^{-jk_ox}$ and letting $x\rightarrow x-x'$ and $x\rightarrow z-z'$ yields the full Green's function.
\begin{equation}
\begin{gathered}
G= \sqrt{\frac{j}{8\pi k_o(x-x')}}\exp\left[-j\frac{k_o(z-z')^2}{2(x-x')} \right]e^{-jk_ox}\\
\boxed{G= \sqrt{\frac{j}{8\pi k_o(x-x')}}\exp\left[-jk_o\left(x + \frac{k_o(z-z')^2}{2(x-x')}\right) \right]}\\
\end{gathered}
\label{mp_eq:11fa}
\end{equation}
