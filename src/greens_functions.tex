...
As we will see shortly, this choice of time dependence drives the sign of the spatial phase dependence and defines the forward and reverse Fourier transforms in the temporal domain.

\iftoggle{separate_greens_functions}{\chapter}{\section} {Green's Identities}
...
\iftoggle{separate_greens_functions}{\chapter}{\section} {Method of Green's Functions} \label{gf_sec:method}
...
Equation \ref{gf_eq:7} is the Helmholtz equation and it represents the wave equation with only spatial derivatives. If we use the wavevector, $k = \omega/c$, and suppress the variable dependence for clarity, Equation \ref{gf_eq:7} can be rewritten as:
...
Equation \ref{gf_eq:9} shows that the Green's function is the solution to the differential equation for a spatial impulse and therefore represents the impulse response of the system. In electromagnetics, the driving force often has a negative sign as in the Laplace equation, $\nabla^2\varphi = -\rho/\epsilon_o$, so we typically chose a matching sign for the delta function. With this convention, we will need to make sure the driving force is applied with a negative sign when utilizing a Green's function found in this manner.
...
\iftoggle{separate_greens_functions}{\chapter}{\section} {Boundary Conditions}
...
\iftoggle{separate_greens_functions}{\section}{\subsection} {Dirichlet Boundary Conditions}
...
\iftoggle{separate_greens_functions}{\section}{\subsection} {Neuman Boundary Conditions}
...
Equation \ref{gf_eq:16} demonstrates the jump condition for the normal derivative of the Green's function. Note that when the surface is taken out to infinity that $\partial G/\partial n = 0$. Green's functions specified in this manner are often called Green's functions of the second kind. Substituting Equation \ref{gf_eq:16} into Equation \ref{gf_eq:11} results in the following equation for $\varphi$:
...
\iftoggle{separate_greens_functions}{\section}{\subsection} {Robin Boundary Conditions}
...
\begin{equation}
\frac{\partial G\left(\mathbf{r}\right)}{\partial n}|_{S} = -\alpha G
\label{gf_eq:18aa}
\end{equation}
\renewcommand{\baselinestretch}{2} \small\normalsize
...
\iftoggle{separate_greens_functions}{\chapter}{\section} {Properties of Green's Functions} \label{gf_sec:properties}
...
\iftoggle{separate_greens_functions}{\section}{\subsection} {Causality} \label{gf_sec:causality}
...
\iftoggle{separate_greens_functions}{\section}{\subsection} {Reciprocity} \label{gf_sec:reciprocity}
...
\iftoggle{separate_greens_functions}{\section}{\subsection} {Continuity} \label{gf_sec:continuity}
A necessary condition for reciprocity is that Green's functions must be continuous in all space. However, they are typically singular so the derivative will generally have a discontinuity at $\mathbf{r} = \mathbf{r}'$.

\iftoggle{separate_greens_functions}{\section}{\subsection} {General Forms}
...
\iftoggle{separate_greens_functions}{\chapter}{\section} {Free Space Green's Functions}
This section demonstrates solving for the free space Green's functions for the Helmholtz equation (Equation \ref{gf_eq:9}) in both 3-dimensions and 2-dimensions. In the 2-dimensional case, we will also derive the Green's function for the Paraxial wave equation. In free space, there are no boundaries, so we can let $r\rightarrow \infty$ and use Dirichlet boundary conditions at $\infty$. Without loss of generality, we can place the delta function at the origin, so that $\delta\left(\mathbf{r}-\mathbf{r}' \right) \rightarrow \delta \left(\mathbf{r} \right)$. To find the Green's function, we need to solve the homogeneous equation away from the delta function and then integrate over a small region around the origin for normalization. The homogenous equation to solve is:
...
\iftoggle{separate_greens_functions}{\section}{\subsection} {3-Dimensions}\label{gf_sec:3d}
...
\iftoggle{separate_greens_functions}{\subsection}{\subsubsection} {Deriviation in the Frequency Domain}
...
\iftoggle{separate_greens_functions}{\subsection}{\subsubsection} {Conversion to the Time Domain}
...
In the time domain, we generally refer to $G_o\left(\mathbf{r},\mathbf{r}',t, t'\right)$ as the retarded wave solution because an observer will experience the source as if it acted at an earlier (retarded) time, $t_1=|t-t'|-\frac{|\mathbf{r}-\mathbf{r}'|}{c}$.

\iftoggle{separate_greens_functions}{\section}{\subsection} {2-Dimensions}\label{gf_sec:2d}
...
From Equation \ref{gf_eq:40g}, $g(x) = g(\tau) = t - \rho\cosh(\tau)/c$ and the only zero  is $\tau = \cosh^{-1}\left(ct/\rho\right)$. We can now rewrite the delta function from Equation \ref{gf_eq:40g} as:
 \begin{equation}
...
\iftoggle{separate_greens_functions}{\section}{\subsection} {Paraxial Wave Equation} \label{gf_sec:paraxial}
In this section we derive the Green's function for the paraxial wave equation.

\iftoggle{separate_greens_functions}{\subsection}{\subsubsection} {Derivation of the Paraxial Wave Equation}
...
\end{equation}
 \renewcommand{\baselinestretch}{2} \small\normalsize
The paraxial wave equation is a parabolic differential equation with the same structure as the diffusion equation. 
...
\iftoggle{separate_greens_functions}{\subsection}{\subsubsection} {Green's Function Derivation in the Frequency Domain}
...
\begin{equation}
G= \sqrt{\frac{j}{8\pi k_ox}}\exp\left[-j\frac{k_oz^2}{2x} \right]e^{-jk_ox}
\label{mp_eq:11fa}
\end{equation}
 
We can let $x\rightarrow |x-x'|$ and $x\rightarrow |z-z'|$ to represent the Green's function as traditionally shown.
\begin{equation}
\boxed{G\left(x,x',z,z' \right)= \sqrt{\frac{j}{8\pi k_o|x-x'|}}\exp\left[-jk_o\left(|x-x'| + \frac{|z-z'|^2}{2|x-x'|}\right) \right]}\\
\label{mp_eq:11fb}
\end{equation}
...
\begin{figure}[H]
\begin{center}
\includegraphics[width=3in]{../media/2d_paraxial_gf_mag.png}
\end{center}
\renewcommand{\baselinestretch}{1}
\small\normalsize
\begin{quote}
...
\begin{figure}[H]
\begin{center}
\includegraphics[width=3in]{../media/2d_paraxial_gf_re_im.png}
\end{center}
\renewcommand{\baselinestretch}{1}
\small\normalsize
\begin{quote}
...
\iftoggle{separate_greens_functions}{\subsection}{\subsubsection} {Derivation from the 3-D Green's Function}

\iftoggle{separate_greens_functions}{\chapter}{\section} {Tabularized Green's Functions}
...
\begin{table}[H]
  ...
  3-D Free Space & $\frac{e^{-jk|\mathbf{r} - \mathbf{r}'|}}{4\pi |\mathbf{r} - \mathbf{r}'|}$ &  $\frac{\delta\left(|t-t'|-\frac{|\mathbf{r}-\mathbf{r}'|}{c} \right)}{4\pi |\mathbf{r}-\mathbf{r}'|}$ \\ \hline
  2-D Free Space & $-\frac{j}{4}H_0^{(2)}\left(k|\boldsymbol{\rho} - \boldsymbol{\rho}' | \right)$ & $\frac{H\left(|t-t'| -\frac{|\boldsymbol{\rho} - \boldsymbol{\rho}'|}{c}\right)}{2\pi \sqrt{|t-t'|^2 -\left(\frac{|\boldsymbol{\rho} - \boldsymbol{\rho}'|}{c}\right)^2 }     }$  \\ \hline
  2-D Paraxial & $\sqrt{\frac{j}{8\pi k|x-x'|}}\exp\left[-jk\left(|x-x'| + \frac{|z-z'|^2}{2|x-x'|}\right) \right]$ & N/A \\ \hline
...
\iftoggle{separate_greens_functions}{\chapter}{\section} {Propagation With Green's Functions}

\iftoggle{separate_greens_functions}{\section}{\subsection} {Free Space Propagation}

\iftoggle{separate_greens_functions}{\section}{\subsection} {Diffraction through an Aperture}

\iftoggle{separate_greens_functions}{\section}{\subsection} {Diffraction from a Planar Surface}

\iftoggle{separate_greens_functions}{\subsection}{\subsubsection} {Integration Along the Surface}

\iftoggle{separate_greens_functions}{\subsection}{\subsubsection} {Integration Perpendicular to the Surface}

\iftoggle{separate_greens_functions}{\section}{\subsection} {Optical Propagation Through Turbulence}
