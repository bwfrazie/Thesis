...
Equation \ref{gf_eq:16} demonstrates the jump condition for the normal derivative of the Green's function. Note that when the surface is taken out to infinity that $\partial G/\partial n = 0$. Substituting Equation \ref{gf_eq:16} into Equation \ref{gf_eq:11} results in the following equation for $\varphi$ in the presence of Neuman boundary conditions.
...
\begin{equation}
G_o\left(\mathbf{r},\mathbf{r}',t\right) = \frac{\delta\left(t-\frac{|\mathbf{r}-\mathbf{r}'|}{c} \right)}{4\pi |\mathbf{r}-\mathbf{r}'|}
\label{gf_eq:31}
\end{equation}
\renewcommand{\baselinestretch}{2} \small\normalsize
...
\begin{equation}
\boxed{G_o\left(\mathbf{r},\mathbf{r}',t,t'\right) = \frac{\delta\left(|t-t'|-\frac{|\mathbf{r}-\mathbf{r}'|}{c} \right)}{4\pi |\mathbf{r}-\mathbf{r}'|}}
\label{gf_eq:32}
\end{equation}
\renewcommand{\baselinestretch}{2} \small\normalsize
...
Equation \ref{gf_eq:44} was derived for rectilinear propagation in a vacuum where the index of refraction, $n$, is equal to $1$. To add the depended on the index of refraction, we need to modify the starting Helmholtz equation to $[\nabla^2 + k_o^2n^2]U = 0$. Following the procedure outline above results in the paraxial equation as given below.

\begin{equation}
-2jk_o\frac{\partial A}{\partial x} + \frac{\partial^2 A}{\partial z^2} + k_o^2\left(n^2-1 \right)A = 0 \\
\label{gf_eq:44aaaa}
\end{equation}
\renewcommand{\baselinestretch}{2} \small\normalsize
...
\begin{equation}
\hat{G_p}= \frac{1}{2jk_o}e^{\frac{jk^2}{2k_o}x}
\label{gf_eq:11cc}
\end{equation}
 \renewcommand{\baselinestretch}{2} \small\normalsize
...
\noindent Equation \ref{gf_eq:11zd} is identical to Equation \ref{gf_eq:11fa} so we get the same answer either way.
...
The only difference between Equation \ref{gf_eq:75} and Equation \ref{gf_eq:44aaaa} is the inclusion of the term $2z/a_e$ that represents the impact of the curvature of the Earth. We can make a substitution for the modified refractivity, $m = n^2-1 + 2z/a_e$, and treat it as a constant. 

\begin{equation}
-2jk_o\frac{\partial A}{\partial x} + \frac{\partial^2 A}{\partial z^2} +k_0^2mA = 0 \\
\label{gf_eq:76}
\end{equation}
\renewcommand{\baselinestretch}{2} \small\normalsize

\subsubsection{Green's Function Derivation in the Frequency Domain}
The derivation of the Green's function for the PWE follows directly from Section \ref{gf_sec:paraxial_gf_derivation}. The Fourier transform of the equation for the Green's function is now given by

\begin{equation}
2jk_o\frac{\partial \hat{G}_p}{\partial x} + (k^2-k_o^2m)\hat{G}_p = \delta(x) \\
\label{gf_eq:200}
\end{equation}
\renewcommand{\baselinestretch}{2} \small\normalsize

The solution in the spatial frequency domain will now have an additional phase factor driven by the correction factor.

\begin{equation}
\begin{aligned}
\hat{g}_p &= \frac{1}{2jk_0}e^{j\frac{(k^2-k_o^2m}{2k_o}x}\\
&=\frac{1}{2jk_0}e^{\frac{jk^2}{2k_o}x} e^{\frac{-jk_om}{2}x}\\
\end{aligned}
\label{gf_eq:201}
\end{equation}
\renewcommand{\baselinestretch}{2} \small\normalsize

Because the additional phase factor does not depend on $k$, it can be pulled out of the integral when taking the inverse Fourier transform. The final Green's function for the PWE is then given as
...
\begin{table}[ht]
  ...
  2-D Paraxial & $\displaystyle\sqrt{\frac{1}{8\pi jk_o|x-x'|}}\exp\left[-jk_o\left(|x-x'| + \frac{|z-z'|^2}{2|x-x'|}\right) \right]$\\ \hline
  2-D PWE &$\displaystyle\sqrt{\frac{1}{8\pi jk_o|x-x'|}}\exp\left[-jk_o\left(|x-x'|\left[1+\frac{m}{2}\right] + \frac{|z-z'|^2}{2|x-x'|}\right) \right]$ \\ \hline
\end{tabular}
\end{center}
\end{table}
\renewcommand{\baselinestretch}{2} \small\normalsize
...
This section describes how to use Green's functions for propagation problems. We will identify the disturbance at the observation point as $U(P_2)$ and the disturbance at the source as $U(P_1)$.
...
=\cos(\theta_1)\left(-jk_o - \frac{1}{r_1} \right)\frac{\exp[-jk_or_1]}{4\pi r_1} -\cos(\theta_2)\left(-jk_o - \frac{1}{r_2} \right)\frac{\exp[-jk_or_2]}{4\pi r_2}
\label{gf_eq:50}
\end{equation}
\renewcommand{\baselinestretch}{2} \small\normalsize
...
This approach only captures the reflected portion of the field, to get the solution for the full field, we need to use superposition and add the solution for the direct path.

\begin{equation}
\begin{aligned}
U_2(P_2) &= U_1(P_2) + U_2(P_2)\\
&= \sqrt{\frac{1}{8\pi jk_oL}}\exp\left[-jk_oL_1\right] + \frac{1}{4\pi}\int_0^L dx \Gamma \sqrt{\frac{1}{x}}\sqrt{\frac{1}{L-x}}\frac{h_2-s}{L-x}\exp\left[-jk_o\left(L_2+L_3\right) \right]
\label{gf_eq:58a}
\end{aligned}
\end{equation}
\renewcommand{\baselinestretch}{2} \small\normalsize
...
We can simplify the various path lengths in Figure \ref{gf_fig:15} through a binomial expansion. Here $L_m$ represents the minimum reflected path, which would be given as a straight line between $P_1$ and the mirror image of $P_2$. 
...
\noindent The path length for a given reflected ray, $L_r$ is given by
...
\noindent We can now substitute this into the exponential term of Equation \ref{gf_eq:58}
...
\begin{equation}
U_2(P_2) = \sqrt{\frac{1}{8\pi jk_o L}}\Gamma_1e^{-jk_oL_m} \sum_{l=-\infty}^{\infty}J_l(\sigma)e^{j\phi_l}
\label{gf_eq:73}
\end{equation}
\renewcommand{\baselinestretch}{2} \small\normalsize
...
\begin{equation}
\boxed{U(P_2) = \sqrt{\frac{1}{8\pi jk_o L}}\left[e^{-jk_oL_1}+\Gamma_1e^{-jk_oL_m} \sum_{l=-\infty}^{\infty}J_l(\sigma)e^{j\phi_l}\right]}
\label{gf_eq:73a}
\end{equation}
\renewcommand{\baselinestretch}{2} \small\normalsize
...
\subsection{Fourier Method for PWE}
...
\subsection{Derivation from PWE}
