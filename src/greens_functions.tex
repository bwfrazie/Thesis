...
The geometric spreading of an analytical electromagnetic wave is dependent on the dimensionality and approximations used. A plane wave is easy to work with but is not physically realizable and shows no spreading or amplitude reduction as it propagates. A paraxial wave allows us to represent beam like waves and will have spreading proportional to $1/\sqrt{L_x}$, where $L_x$ is the length along the propagation direction (this can be interpreted as the ground distance between points). A cylindrical wave will have spreading proportional to $1/\sqrt{\rho}$ and a spherical wave will have spreading proportional to $1/r$, where $\rho$ and $r$ are the respective path lengths in the corresponding coordinate system. The amount of geometry induced spreading increases as we add higher fidelity to our models and analytical tools so we need to take that into consideration when comparing models and try to remain consistent wherever possible.

\subsection{Common Pitfalls}
There are several potential issues that need to be considered when applying numerical methods to any propagation problem. The following list is certainly not exhaustive, but captures some of the biggest tripping points that may be encountered.

\begin{enumerate}
\item Consistency with Fourier transform definitions. The time and space versions of Fourier transforms will have opposing signs due to causality. When using numerical Fast Fourier Transform (FFT) implementations, we need to ensure they match the conventions used as described in Table \ref{gf_tab:0a}. If they do not agree, a general fix is to take the complex conjugate of any exponential terms in the analytical expression.
\item Polarization effects are not captured in scalar propagation. Because we are restricting ourselves to scalar analysis, there is no sense of polarization. This means that once set, the polarization will be constant for all time. Polarization is accounted for through the reflection coefficients and boundary conditions as those implicitly have polarization dependence.
...
For the purposes of this document, we are interested in time varying propagation problems rather than electrostatic ones. Therefore, we would like to apply the method of Green's functions to the scalar wave equation  to solve for the scalar field $\varphi$ as a function of space and time with an arbitrary driving function, $f$, that is also a function of space and time:
...
\begin{equation}
\nabla^2G+ k_o^2n_r^2G = -\delta\left(\mathbf{r}-\mathbf{r}' \right)
\label{gf_eq:9}
\end{equation}
\renewcommand{\baselinestretch}{2} \small\normalsize
...
Equations \ref{gf_eq:8} and \ref{gf_eq:9} can be rearranged to solve for the Laplacian, so that $\nabla^2\varphi = f - k_o^2n_r^2\varphi$ and $\nabla^2G = -\delta\left(\mathbf{r}-\mathbf{r}' \right) - k_o^2n_r^2G$. We can substitute these values into Green's second identity (Equation \ref{gf_eq:5}), with $\psi=G$:
...
\oint\limits_{S} \left[ \varphi\frac{\partial G}{\partial n} - G\frac{\partial \varphi}{\partial n} \right]dS = \int\limits_{V}d^3r' \left[ \varphi \left(-\delta\left(\mathbf{r}-\mathbf{r}' \right) - k_o^2n_r^2G\right)- G \left(f - k_o^2n_r^2\varphi \right)\right] \\
\int\limits_{V}d^3r'\varphi\delta\left(\mathbf{r}-\mathbf{r}' \right) = \oint\limits_{S}\left[G\frac{\partial \varphi}{\partial n} - \varphi\frac{\partial G}{\partial n} \right]dS +\int\limits_{V}d^3r'\left[ Gk_o^2n_r^2\varphi - \varphi k_o^2n_r^2G - Gf \right] \\
...
A necessary condition for reciprocity is that Green's functions must be continuous in all space. However, they are singular at so the derivative will have a discontinuity at $\mathbf{r} = \mathbf{r}'$.
...
\begin{equation}
G\left(\mathbf{r},\mathbf{r}',t ,t'\right) = g\left(\omega |t-t'| - k_on_r| \mathbf{r} - \mathbf{r}' | \right)
\label{gf_eq:19b}
\end{equation}
\renewcommand{\baselinestretch}{2} \small\normalsize
...
\begin{equation}
\frac{1}{r^2}\frac{\partial}{\partial r}\left(r^2\frac{\partial G}{\partial r}\right)+ k_o^2n_r^2G = 0
\label{gf_eq:20}
\end{equation}
\renewcommand{\baselinestretch}{2} \small\normalsize
...
\begin{equation}
\begin{gathered}
\frac{1}{r^2}\left[2r\frac{\partial G}{\partial r}+ r^2\frac{\partial^2 G}{\partial r^2}  \right] + k_o^2n_r^2G = 0 \\
r\frac{\partial^2 G}{\partial r^2} + 2\frac{\partial G}{\partial r}+ k_o^2n_r^2rG = 0 \\
\frac{\partial^2 \left(rG\right)}{\partial r^2} + k_o^2n_r^2rG = 0
\end{gathered}
\label{gf_eq:21}
\end{equation}
\renewcommand{\baselinestretch}{2} \small\normalsize
...
\begin{equation}
G = c_1\frac{e^{jk_on_rr}}{r} + c_2\frac{e^{-jk_on_rr}}{r}
\label{gf_eq:22}
\end{equation}
\renewcommand{\baselinestretch}{2} \small\normalsize
...
\begin{equation}
\begin{gathered}
\lim_{r\to0}\int\limits_{V}d^3r'\left[\nabla^2G+ k_o^2n_r^2G\right] = -\lim_{r\to0}\int\limits_{V}\delta\left(\mathbf{r}\right) \\
\lim_{r\to0}\int\limits_{V}d^3r'\left[\nabla \cdot\nabla G+ k_o^2n_r^2G\right] = -1 \\
\lim_{r\to0}\left[\oint\limits_{S}\nabla G \cdot \hat{n} dS + \int\limits_{V}d^3r' k_o^2n_r^2G\right] = -1 \\
\lim_{r\to0}\left[\oint\limits_{S}\frac{\partial G}{\partial n} dS + \int\limits_{V}d^3r' k_o^2n_r^2G\right] = -1
\end{gathered}
\label{gf_eq:24}
\end{equation}
\renewcommand{\baselinestretch}{2} \small\normalsize
...
\begin{equation}
\begin{gathered}
\lim_{r\to0}\left[c_2\oint\limits_{S}\frac{\partial }{\partial r'}\left(\frac{e^{-jk_on_rr'}}{r'}\right) r'^2\sin{\theta'} d\theta' d\phi '+ c_2\int\limits_{V}r'^2 \sin{\theta'}dr' d\theta' d\phi' k_o^2n_r^2\frac{e^{-jk_on_rr'}}{r'}\right] = -1 \\
\lim_{r\to0}\left[c_2 r^2\left( -\frac{jk_on_r}{r} - \frac{1}{r^2}\right)e^{-jk_on_rr}\oint\limits_{S}\sin{\theta'} d\theta' d\phi' + 4\pi c_2k_o^2n_r^2\int dr' r'e^{-jk_or'}\right] = -1 \\
\lim_{r\to0}\left[4\pi c_2 \left( -jk_on_rr - 1\right)e^{-jk_on_rr}+ 4\pi c_2k_o^2n_r^2\int dr' \frac{\partial }{\partial k_o}\frac{e^{-jk_on_rr'}}{-jn_r}\right] = -1 \\
\lim_{r\to0}4\pi c_2\left[\left( -jk_or - 1\right)e^{-jk_on_rr}+ j4\pi c_2k_o^2n_r\frac{\partial }{\partial k_o}\int dr'e^{-jk_on_rr'} \right] = -1 \\
4\pi c_2\left[-1 +  \lim_{r\to0}j k_o^2n_r\frac{\partial}{\partial k_o}\frac{1}{-jk_on_r}e^{-jk_on_rr'}\bigg|_o^r \right] = -1 \\
-4\pi c_2\left[1 - \lim_{r\to0}k_o^2\frac{\partial}{\partial k_o}\frac{1}{k_o}\left(e^{-jk_on_rr} -1\right)\right] = -1 \\
c_2\left[1 - \lim_{r\to0}k_o^2\left(-\frac{1}{k_o^2}e^{-jk_on_rr} -\frac{jn_rr}{k_o}e^{-jk_on_rr}+\frac{1}{k_o^2}\right)\right] = \frac{1}{4\pi} \\
 c_2 = \frac{1}{4\pi}
\end{gathered}
\label{gf_eq:25}
\end{equation}
...
\begin{equation}
\boxed{G_o\left(\mathbf{r},\mathbf{r}'\right) = \frac{e^{-jk_on_r|\mathbf{r} - \mathbf{r}'|}}{4\pi |\mathbf{r} - \mathbf{r}'|}}
\label{gf_eq:26}
\end{equation}
\renewcommand{\baselinestretch}{2} \small\normalsize
...
\begin{equation}
G_o\left(\mathbf{r},\mathbf{r}'\right)  = \frac{e^{-jk_on_r|\mathbf{r} - \mathbf{r}'|}}{|\mathbf{r} - \mathbf{r}'|}
\label{gf_eq:27}
\end{equation}
\renewcommand{\baselinestretch}{2} \small\normalsize
...
\begin{equation}
\frac{1}{\rho}\frac{\partial}{\partial \rho}\left(\rho\frac{\partial G}{\partial \rho}\right)+ k_o^2n_r^2G = 0
\label{gf_eq:33}
\end{equation}
\renewcommand{\baselinestretch}{2} \small\normalsize
...
\begin{equation}
\frac{1}{\rho}\frac{\partial}{\partial \rho}\left(\rho\frac{\partial G}{\partial \rho}\right)+ \left(k_o^2n_r^2 -\nu^2\right)G = 0
\label{gf_eq:34}
\end{equation}
\renewcommand{\baselinestretch}{2} \small\normalsize
...
\begin{equation}
G = c_1J_0\left(k_on_r\rho\right) + c_2N_0\left(k_on_r\rho\right)
\label{gf_eq:35}
\end{equation}
\renewcommand{\baselinestretch}{2} \small\normalsize
...
\begin{equation}
\begin{gathered}
H_0^{(1)}(k\rho) = J_0(k_on_r\rho) + jN_0(k_on_r\rho) \\
H_0^{(2)}(k\rho) = J_0(k_on_r\rho) - jN_0(k_on_r\rho)
\label{gf_eq:36}
\end{gathered}
\end{equation}
\renewcommand{\baselinestretch}{2} \small\normalsize
...
\begin{equation}
\begin{gathered}
H_0^{(1)}(k_o\rho) \approx \sqrt{\frac{2}{\pi k_on_r\rho}}e^{j\left(k_on_r\rho - \frac{\pi}{4}\right)}\\
H_0^{(2)}(k_o\rho) \approx \sqrt{\frac{2}{\pi k_on_r\rho}}e^{-j\left(k_on_r\rho - \frac{\pi}{4}\right)}
\label{gf_eq:36a}
\end{gathered}
\end{equation}
\renewcommand{\baselinestretch}{2} \small\normalsize
...
\begin{equation}
G = c_1H_0^{(1)}\left(k_on_r\rho\right) +c_2H_0^{(2)}\left(k_on_r\rho\right) 
\label{gf_eq:37}
\end{equation}
\renewcommand{\baselinestretch}{2} \small\normalsize
...
\begin{equation}
G = c_2H_0^{(2)}\left(k_on_r\rho\right) 
\label{gf_eq:38}
\end{equation}
\renewcommand{\baselinestretch}{2} \small\normalsize

As in Section \ref{gf_sec:3d}, we integrate Equation \ref{gf_eq:9} over a small disk, $D$, centered at $\rho = 0$ and take the limit as $\rho \rightarrow 0$. For small arguments, the asymptotic behavior of $H_0^{(2)}(k_on_r\rho) \sim -j\frac{2}{\pi}\ln\left({k_on_r\rho}\right)$ and we can substitute $G = -\frac{j2c_2}{\pi}\ln\left({k_on_r\rho}\right)$ \cite{abramowitz_stegun}. 

\begin{equation}
\begin{gathered}
\lim_{\rho\to 0}\int\limits_{D} \left[ \frac{1}{\rho'}\frac{\partial}{\partial \rho'}\left(\rho' \frac{\partial G}{\partial \rho'} \right) + k_o^2n_r^2G\right]\rho' d\rho' d\theta' = \lim_{\rho\to 0}\int\limits_{D} \delta\left(\boldsymbol{\rho}-\boldsymbol{\rho}' \right)\rho' d\rho' d\phi' \\
\lim_{\rho\to 0}\int\limits_{D} \left[ -\frac{1}{\rho'}\frac{\partial}{\partial \rho'}\left(\rho' jc_1\frac{2}{\pi}\frac{\partial \ln(k_on_r\rho')}{\partial \rho'} \right) - k_o^2n_r^2jc_1\frac{2}{\pi}\ln(k_on_r\rho')\right]\rho' d\rho' d\theta' = -1 \\
\lim_{\rho\to 0}-4jc_1\int\limits_{D} \left[\frac{\partial}{\partial \rho'}\left(\rho' \frac{\partial \ln(k_on_r\rho')}{\partial \rho'} \right) + k_o^2n_r^2\rho'\ln(k_on_r\rho')\right] d\rho' = -1 \\
\lim_{\rho\to 0}4jc_1\left[\left(\rho' \frac{1 }{\rho'} \right)\bigg|_0^{\rho} + \int\limits_{D}k_o^2n_r^2\rho'\ln(k_on_r\rho') d\rho'\right] = 1 \\
\lim_{\rho\to 0}c_1\left[ 1 +  k_o^2n_r^2\left( \ln(k_on_r\rho')\frac{\rho'^2}{2}\bigg|_0^{\rho} - \int\limits_{D}\frac{\rho'^2}{2}\frac{1}{\rho'} d\rho' \right)\right] = -\frac{j}{4} \\
\lim_{\rho\to 0}c_1\left[ 1 +  k_o^2n_r^2\left( \ln(k_on_r\rho)\frac{\rho^2}{2} - \frac{1}{2}\int\limits_{D}\rho' d\rho' \right)\right] = -\frac{j}{4} \\
...
\begin{equation}
\boxed{G_o\left(\boldsymbol{\rho},\boldsymbol{\rho}'\right) = -\frac{j}{4}H_0^{(2)}\left(k_on_r|\boldsymbol{\rho} - \boldsymbol{\rho}' | \right)}
\label{gf_eq:40}
\end{equation}
\renewcommand{\baselinestretch}{2} \small\normalsize
...
We can derive the paraxial from the Helmholtz equation by assuming a wave propagating in the $x$ direction of the form $U(x,z) = A(x,z)e^{-jk_ox}$. We will see that by assuming the propagating wave is not dependent on the index of refraction, the Green's function dependence on the index of refraction can be easily factored out into a phase term.

 \begin{equation}
 \begin{gathered}
 \left[ \nabla^2 + k_o^2n_r^2\right]U = 0 \\
\left[\frac{\partial^2 }{\partial x^2} + \frac{\partial^2 }{\partial z^2} + k_o^2n_r^2\right]U = 0 \\
\frac{\partial }{\partial x}\left(\frac{\partial U}{\partial x} \right) + \frac{\partial^2 U}{\partial z^2} + k_o^2n_r^2 U = 0 \\
\frac{\partial }{\partial x}\left(-jk_oAe^{-jk_ox}+e^{-jk_ox}\frac{\partial A}{\partial x} \right) + e^{-jk_ox}\frac{\partial^2 A}{\partial z^2} + k_o^2n_r^2 Ae^{-jk_ox} = 0 \\
-k_o^2Ae^{-jk_ox} -2jk_oe^{-jk_ox}\frac{\partial A}{\partial x}+e^{-jk_ox}\frac{\partial^2 A}{\partial x^2} + e^{-jk_ox}\frac{\partial^2 A}{\partial z^2} + k_o^2n_r^2 Ae^{-jk_ox} = 0 \\
e^{-jk_ox}\left[ -2jk_o\frac{\partial A}{\partial x}+\frac{\partial^2 A}{\partial x^2} + \frac{\partial^2 A}{\partial z^2} + k_o^2\left(n_r^2-1\right)A\right] = 0 \\
\end{gathered}
\label{gf_eq:41}
\end{equation}
 \renewcommand{\baselinestretch}{2} \small\normalsize
...
\noindent This gives us an equation dependent only on the amplitude, $A$
 ...
 \begin{equation}
-2jk_o\frac{\partial A}{\partial x}+\frac{\partial^2 A}{\partial x^2} + \frac{\partial^2 A}{\partial z^2} + k_o^2\left(n_r^2-1\right)A= 0 \\
\label{gf_eq:42}
\end{equation}
 \renewcommand{\baselinestretch}{2} \small\normalsize
...
\begin{equation}
-2jk_o\frac{\partial G_p}{\partial x} + \frac{\partial^2 G_p}{\partial z^2} +k_o^2(n_r^2-1)\hat{G_p}= -\delta(x)\delta(z)
\label{gf_eq:44a}
\end{equation}
 \renewcommand{\baselinestretch}{2} \small\normalsize
...
\begin{equation}
2jk_o\frac{\partial \hat{G_p}}{\partial x} +k^2\hat{G_p} +k_o^2(n_r^2-1)\hat{G_p}= \delta(x)
\label{gf_eq:45}
\end{equation}
 \renewcommand{\baselinestretch}{2} \small\normalsize
...
\begin{equation}
\hat{G_p}= ce^{\frac{jk^2}{2k_o}x}e^{\frac{-jk_o(n_r^2-1)}{2}x}
\label{gf_eq:46}
\end{equation}
 \renewcommand{\baselinestretch}{2} \small\normalsize
...
\begin{equation}
\lim_{x\rightarrow 0}\int_{-\epsilon}^{\epsilon}dx\left[2jk_o\frac{\partial\hat{G_p}}{\partial x}+ k^2\hat{G_p} +k_o^2(n_r^2-1)\hat{G_p} \right] = \lim_{x\rightarrow 0}\int_{-\epsilon}^{\epsilon}dx\delta(x)\\
\label{gf_eq:47}
\end{equation}
 \renewcommand{\baselinestretch}{2} \small\normalsize
 
\noindent $\hat{G_p}$ must be continuous, so the integral over the $k^2\hat{G_p}$ and $k_0^2(n_r^2-1)\hat{G_p}$ terms must be 0.

\begin{equation}
\begin{gathered}
\lim_{x\rightarrow 0}2jk_o\int_{-\epsilon}^{\epsilon}dx\frac{\partial\hat{G_p}}{\partial x}= 1\\
\lim_{x\rightarrow 0}2jk_o\hat{G_p} = 1 \\
\lim_{x\rightarrow 0}2jk_oce^{\frac{jk^2}{2k_o}x}e^{\frac{-jk_o(n_r^2-1)}{2}x} = 1\\
c = \frac{1}{j2k_o}\\
\end{gathered}
\label{gf_eq:11cb}
\end{equation}
...
\begin{equation}
\hat{G_p}= \frac{1}{2jk_o}e^{\frac{jk^2}{2k_o}x}e^{\frac{-jk_o(n_r^2-1)}{2}x}
\label{gf_eq:11cc}
\end{equation}
 \renewcommand{\baselinestretch}{2} \small\normalsize
...
\begin{equation}
\begin{aligned}
G_p &= \mathcal{F}^{-1}\{\hat{G_p}\} = \frac{1}{2jk_o}\frac{1}{2\pi}\int_{-\infty}^{\infty}dk e^{\frac{jk^2}{2k_o}x}e^{-\frac{jk_o(n_r^2-1)}{2}x}e^{-jkz} \\
& = \frac{1}{2jk_o}\frac{1}{2\pi}e^{-\frac{jk_o(n_r^2-1)}{2}x}\int_{-\infty}^{\infty}dk e^{\frac{jk^2}{2k_o}x-jkz} \\
\end{aligned}
\label{gf_eq:11d}
\end{equation}
 \renewcommand{\baselinestretch}{2} \small\normalsize
...
\begin{equation}
\begin{aligned}
G_p &= \frac{1}{2jk_o}\frac{1}{2\pi}e^{-\frac{jk_o(n_r^2-1)}{2}x}\int_{-\infty}^{\infty}dk e^{\frac{jx}{2k_o}\left(k  -\frac{k_oz}{x}\right)^2- \frac{jk_o}{2x}z^2 } \\
&= \frac{1}{2jk_o}\frac{1}{2\pi} \sqrt{\frac{\pi j2k_o}{x}}e^{-\frac{jk_o(n_r^2-1)}{2}x}e^{-j\frac{k_o}{2x}z^2 } \\
&= \frac{1}{2jk_o}\sqrt{\frac{jk_o}{2\pi x}}\exp\left[-\frac{jk_o(n_r^2-1)}{2}x\right]\exp\left[-j\frac{k_oz^2}{2x} \right]\\
&= \sqrt{\frac{1}{8\pi j k_ox}}\exp\left[-\frac{jk_o(n_r^2-1)}{2}x\right]\exp\left[-j\frac{k_oz^2}{2x} \right]\\
\end{aligned}
\label{gf_eq:11f}
\end{equation}
 \renewcommand{\baselinestretch}{2} \small\normalsize
...
\begin{equation}
G= \sqrt{\frac{1}{8\pi jk_ox}}\exp\left[-\frac{jk_o(n_r^2-1)}{2}x\right]\exp\left[-j\frac{k_oz^2}{2x} \right]e^{-jk_ox}
\label{gf_eq:11fa}
\end{equation}
  \renewcommand{\baselinestretch}{2} \small\normalsize
  
We can let $x\rightarrow |x-x'|$, $x\rightarrow |z-z'|$ and introduce the modified refractivity, $m = n_r^2-1$ to represent the Green's function as traditionally shown.

\begin{equation}
\boxed{G\left(x,x',z,z' \right)= \sqrt{\frac{1}{8\pi jk_o|x-x'|}}\exp\left[-jk_o\left(|x-x'|\left[1+\frac{m}{2}\right] + \frac{|z-z'|^2}{2|x-x'|}\right)\right]}\\
\label{gf_eq:11fb}
\end{equation}
 \renewcommand{\baselinestretch}{2} \small\normalsize
...
It is important to note that with the paraxial approximation, only forward propagating waves are included in the solution. This means that effects such as backscattering are not captured with this method. 
...
\begin{equation}
G_o \approx \frac{\exp\left[-jk_on_r\left(x+\frac{z^2}{2x} \right)\right]}{4\pi x} \exp\left[-jk_on_r\frac{y^2}{2x}\right]
\label{gf_eq:11zb}
\end{equation}
 \renewcommand{\baselinestretch}{2} \small\normalsize
...
\begin{equation}
\int_{-\infty}^{\infty} dy \exp\left[-jk_o\frac{y^2}{2x}\right] = \sqrt{\frac{2\pi x}{jk_on_r}}
\label{gf_eq:11zc}
\end{equation}
 \renewcommand{\baselinestretch}{2} \small\normalsize
...
\begin{equation}
\begin{aligned}
G_o &= \frac{\exp\left[-jk_on_r\left(x+\frac{z^2}{2x} \right)\right]}{4\pi x} \sqrt{\frac{2\pi x}{jk_on_r}}\\
&= \sqrt{\frac{1}{j8\pi k_on_r x}}\exp\left[-jk_on_r\left(x+\frac{z^2}{2x} \right)\right]
\label{gf_eq:11zd}
\end{aligned}
\end{equation}
 \renewcommand{\baselinestretch}{2} \small\normalsize
 
Equation \ref{gf_eq:11zd} is different from Equation  \ref{gf_eq:11fa} in that the dependence on the index of refraction is no longer separable into a phase term. This is however the same answer we would have gotten if we assumed the propagating wave had the form $U(x,z) = A(x,z)e^{-jk_on_rx}$ when deriving the paraxial wave equation.
...
\subsubsection{Derivation of the Parabolic Wave Equation}
...
\begin{equation}
-2jk_o\frac{\partial A}{\partial x} + \frac{\partial^2 A}{\partial z^2} +k_0^2\left(n^2-1+\frac{2z}{a_e} \right)A = 0 \\
\label{gf_eq:75}
\end{equation}
\renewcommand{\baselinestretch}{2} \small\normalsize
...
\begin{equation}
-2jk_o\frac{\partial A}{\partial x} + \frac{\partial^2 A}{\partial z^2} +k_0^2m_zA = 0 \\
\label{gf_eq:76}
\end{equation}
\renewcommand{\baselinestretch}{2} \small\normalsize
...
\begin{equation}
\boxed{G\left(x,x',z,z' \right)= \sqrt{\frac{1}{8\pi jk_o|x-x'|}}\exp\left[-jk_o\left(|x-x'|\left[1+\frac{m_z}{2}\right] + \frac{|z-z'|^2}{2|x-x'|}\right) \right]}\\
\label{gf_eq:202}
\end{equation}
 \renewcommand{\baselinestretch}{2} \small\normalsize
...
\begin{table}[ht]
  ...
  3-D Free Space & $\displaystyle\frac{e^{-jk_on_r|\mathbf{r} - \mathbf{r}'|}}{4\pi |\mathbf{r} - \mathbf{r}'|}$ \\ \hline
  2-D Free Space & $\displaystyle -\frac{j}{4}H_0^{(2)}\left(k_on_r|\boldsymbol{\rho} - \boldsymbol{\rho}' | \right)$  \\ \hline
  2-D Paraxial & $\displaystyle\sqrt{\frac{1}{8\pi jk_o|x-x'|}}\exp\left[-jk_o\left(|x-x'|\left[1+\frac{m}{2}\right] + \frac{|z-z'|^2}{2|x-x'|}\right)\right]$\\ \hline
  2-D PWE &$\displaystyle\sqrt{\frac{1}{8\pi jk_o|x-x'|}}\exp\left[-jk_o\left(|x-x'|\left[1+\frac{m_z}{2}\right] + \frac{|z-z'|^2}{2|x-x'|}\right) \right]$ \\ \hline
\end{tabular}
\end{center}
\end{table}
\renewcommand{\baselinestretch}{2} \small\normalsize
...
\subsection{Planar Surface with Paraxial Approximation}\label{paraxial_diffraction}
...
\subsection{Planar Surface with Cylindrical Waves}\label{cylindrical_diffraction}
The analysis here will rely extensively on the work presented in Section \ref{paraxial_diffraction}, but we will use cylindrical waves instead of paraxial waves. We will again use the geometry shown in Figure \ref{gf_fig:15}.

We will work with the 2-D free space Green's function with the phase induced by the index of refraction factored out.

\begin{equation}
G\left(\boldsymbol{\rho},\boldsymbol{\rho}'\right) = -\frac{j}{4}H_0^{(2)}\left(k_o|\boldsymbol{\rho} - \boldsymbol{\rho}' | \right)\exp\left[-\frac{jk_om}{2}|\boldsymbol{\rho} - \boldsymbol{\rho}' |\right]
\label{cyl_eq:1}
\end{equation}
\renewcommand{\baselinestretch}{2} \small\normalsize
...
&= -j\sqrt{\frac{k_o}{2\pi}}e^{j3\pi /4}\int_0^L dx \Gamma U_2(s)\frac{h_2}{L_3}\frac{1}{\sqrt{L_3}}e^{-jk_oL_3} \\
...
&= -j\sqrt{\frac{1}{8\pi k_oj L_2}}e^{j\pi/4}e^{-jk_oL_2} \\
...
\noindent Substituting in for $x_m$ yields the final asymptotic solution
...
\subsection {Knife Edges and Non-planar Surfaces}
