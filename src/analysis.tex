\chapter{Analytical Propagation Model}
...
The reflected wave solution at the sea surface,$U_2(s)$, follows from the solution to the direct path, with $x = x$ and $z = 0$ at the sea surface.
\begin{equation}
\begin{gathered}
...
We will first focus on simplifying the exponential term, $\kappa$, in Equation \ref{mp_eq:12h}.
\begin{equation}
\begin{aligned}
...
Even with low altitudes, the phase oscillations will be rapid as $\tilde{x}$ goes past the limits of integration and will cancel out, so we can use the principle of stationary phase and let the positive limit extend to $\infty$.
...
= \Gamma_1e^{-j\left(k_oL_{so}+\pi/4\right)}  \sqrt{\frac{h_1+h_2}{h_1}}\sqrt{\frac{k_o}{2\pi j L}}\sqrt{\frac{h_1+h_2}{h_2}}\frac{h_1+h_2}{L}\sqrt{\frac{2\pi h_1h_2L^3}{k_o(h_1+h_2)^4}} \\
= \Gamma_1e^{-j\left(k_oL_{so}+\pi/4\right)}\sqrt{\frac{k_o}{2\pi jL}} \\
\end{gathered}
\label{mp_eq:27}
\end{equation}
...
\begin{equation}
\begin{gathered}
F_p= \frac{1}{2jk_o}\sqrt{\frac{k_0}{2\pi jL}}\left[e^{-jkL_1} + \Gamma_1e^{-j\left(k_oL_{so}+\pi/4\right)}\right] \\
\end{gathered}
\label{mp_eq:27}
\end{equation}
...
\begin{equation}
\begin{gathered}
\boxed{F_p= e^{-jkL_1} + \Gamma_1e^{-j\left(k_oL_{so}+\pi/4\right)}} \\
\end{gathered}
\label{mp_eq:28}
\end{equation}
