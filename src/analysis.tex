\chapter{Paraxial Wave Propagation Model}
...
To simplify this expression, we will first expand the exponential term, $K(x)$.
\begin{equation}
\begin{aligned}
...
Even with low altitudes, the phase oscillations will be rapid as $\tilde{x}$ goes past the limits of integration and will cancel out, so we can use the principle of stationary phase and let the limits extend to $\pm\infty$. For each case shown, the carrier frequency of the electromagnetic wave was set to 35 GHz. Lower frequencies will have fewer oscillations, but the pricinple of stationary phase will still apply.
...
We can solve this equation using the saddle point method as described in Appendix \ref{appendix_saddle_point_method}, where we musst deform the contour to ensure it doesn't pass through the critical point at $x=x_m$. The solution to Equation \ref{mp_eq:23} is then
\begin{equation}
\begin{aligned}
...
The deterministic component of the reflected wave is now
\begin{equation}
\begin{aligned}
...
We can then use the Jacobi-Anger expansion \cite{gbur_math} to convert the exponential term on the far right hand side into sum involving Bessel functions.
\begin{equation}
\begin{aligned}
...
Following the approach for the deterministic component yields the asymptotic solution
\begin{equation}
\begin{aligned}
\boxed{U_2(P_2) = \frac{1}{2jk_o}\sqrt{\frac{k_o}{2\pi jL}}e^{-j\left(k_oL_{so}+\pi/2\right)}\Gamma_1\sum_{l=-\infty}^{\infty}J_l(\sigma)\exp\left[jl\theta\right]}
\label{mp_eq:35}
\end{aligned}
\end{equation}
\renewcommand{\baselinestretch}{2} \small\normalsize
