...
Figure \ref{os_fig:7bc} shows 2-D slices of the power spectrum vs the 1-D power spectrum on the left hand side and the same for the curvature spectrum on the right hand side, where $\Psi_x = \Psi(k_y = 0)$ and $\Psi_u = \Psi(k_x = 0)$. The 2-D spectral slices are plotted as $k_x\Psi_x$ and $k_y\Psi_y$ so that all three spectra have the same units. Note that the $\Psi_x$ slice $\approx S$ and that the $\Psi_y$ slice has a shifted spectral peak and low frequency cutoff but converges to the other spectra at high spatial frequencies.
\begin{figure}[H]
  \begin{center}
...
\begin{equation}
  ...
  \end{cases} 
\end{equation}
The extra factor of $\frac{1}{2}$ in line 2 of Equation \ref{os_eq:8} comes from the fact that we have Hermitian symmetry and need to normalize both the expectations $\left<w_j + iu_j\right>$ and $\left<w_j - iu_j\right>$ to conserve energy. Also note that the power spectrum for frequencies $k = 0$, and $k = \frac{N}{2}$ is not divided by 2 since these are single points that are not repeated.
...
Figure \ref{os_fig:7a} shows an example realization for $U_{10}$ = 10 m/s and $L$ = 1 km. The upper plots show the generated sea surface and a comparison of the original spectrum to the recovered spectrum for $N = 500L$ ($\Delta x$ = 1/500 m). The bottom plots show the same for $N=2L$ ($\Delta x$ = 1/2 m).
\begin{figure}[H]
  \begin{center}
...
Figure \ref{os_fig:7aa} shows another example realization, this time for $L$ = 10 km. The upper plots show the generated sea surface and a comparison of the original spectrum to the recovered spectrum for $N = 500L$ ($\Delta x$ = 1/500 m). The bottom plots show the same for $N=2L$ ($\Delta x$ = 1/2 m).
\begin{figure}[H]
  \begin{center}
...
\begin{equation}
  ...
  V(\alpha,j) = \begin{cases}
    ...
    \frac{1}{2}\sqrt{\frac{1}{2}\Psi(\alpha,j)\Delta k^2}\left[w(j) + iu(j) \right], & 1 \leq j < \frac{N}{2} \\
...
Figure \ref{os_fig:10} shows slices along the $N/2$ element in both the $x$ and $y$ planes on the left hand side and a comparison of the spectral slices in the 2-D Elfouhaily spectrum on the right hand side. The full 2-D spectrum was estimated from the generated surface and then slices were taken to compare to slices from the original 2-D spectrum. The low frequency content does not match as well as in the 1-dimensional case.
\begin{figure}[H]
  \begin{center}
...
Figure \ref{os_fig:8a} shows the same thing as Figure \ref{os_fig:8} but with $N=5L$ instead of $N = 2L$.
\begin{figure}[H]
  \begin{center}
\includegraphics[width=6in]{../media/Ocean_Surface/sea_surface_2d_surf_1000_5.png}
  \end{center}
  \renewcommand{\baselinestretch}{1} \small\normalsize
  \begin{quote}
...
Figure \ref{os_fig:10a} shows the same thing as Figure \ref{os_fig:10} but with $N=5L$ instead of $N = 2L$.
\begin{figure}[H]
  \begin{center}
\includegraphics[width=6in]{../media/Ocean_Surface/sea_surface_2d_slices_1000_5.png}
  \end{center}
  \renewcommand{\baselinestretch}{1} \small\normalsize
  \begin{quote}
...
Figure \ref{os_fig:11} shows the generated sea surface for $L$ = 5 km and $U_{10}$ = 10 m/s. Again, the right hand plot shows an image of a 1000 square meter section of the surface and the left hand plot shows a 3D rendering of the 100 square meter patch outlined in the red box in the upper left hand corner of the left hand plot.
\begin{figure}[H]
  \begin{center}
...
Figure \ref{os_fig:11a} shows slices along the $N/2$ element in both the $x$ and $y$ planes on the left hand side and a comparison of the spectral slices in the 2-D Elfouhaily spectrum on the right hand side. The full 2-D spectrum was estimated from the generated surface and then slices were taken to compare to slices from the original 2-D spectrum. The low frequency content does not match as well as in the 1-dimensional case.
\begin{figure}[H]
  \begin{center}
...
Figure \ref{os_fig:12} shows the generated sea surface for $L$ = 7.5 km and $U_{10}$ = 10 m/s. Again, the right hand plot shows an image of a 1000 square meter section of the surface and the left hand plot shows a 3D rendering of the 100 square meter patch outlined in the red box in the upper left hand corner of the left hand plot.
\begin{figure}[H]
  \begin{center}
...
Figure \ref{os_fig:12a} shows slices along the $N/2$ element in both the $x$ and $y$ planes on the left hand side and a comparison of the spectral slices in the 2-D Elfouhaily spectrum on the right hand side. The full 2-D spectrum was estimated from the generated surface and then slices were taken to compare to slices from the original 2-D spectrum. The low frequency content does not match as well as in the 1-dimensional case.
\begin{figure}[H]
  \begin{center}
