...
\subsubsection{Ensemble Statistics}
Figure \ref{os_fig:7ffg} shows the 2-D surface RMS wave height vs. wind speed. This figure was created using 6 sampling conditions ($L = 1 $ km, $L = 10 $ km, $N = 0.5L$, $N = L$, and $N = 2L$) to generate $100$ realizations of surfaces at wind speeds from $1$ m/s to $20$ m/s. Shown in the plots are the ensemble RMS waves heights along the $x$ direction and along the $y$ direction as well as the total RMS wave heights. The total RMS and the RMS along $x$ are shown in the upper grouping at high wind speed and are consistent with Table 6.3 and Figure 6.6 in \cite{temper_guide}. The RMS along $y$ is shown in the lower grouping at high wind speed and demonstrates the directional dependence of the spectrum.
\begin{figure}[H]
  \begin{center}
