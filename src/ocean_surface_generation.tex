...
Figure \ref{os_fig:7bc} shows 2-D slices of the power spectrum vs the 1-D power spectrum on the left hand side ($\Psi_x = \Psi(k_y = 0)$, $\Psi_u = \Psi(k_x = 0)$) and the same for the curvature spectrum on the right hand side.
\begin{figure}[H]
  \begin{center}
...
\begin{equation}
  ...
  V(\alpha,j) = \begin{cases}
    ...
    \frac{1}{4}\sqrt{\Psi(\alpha,j)\Delta k^2}\left[w(j) + iu(j) \right], & 1 \leq j < \frac{N}{2} \\
   \sqrt{\frac{1}{2}\Psi(\alpha,N/2)\Delta k^2}u(0) & j = \frac{N}{2} \\
