...
Once we have all the pieces, we can lay everything down in the appropriate location in $V$. Again, the sea surface, $H$, is generated through the inverse Fourier transform.
...
The statistics for any given sea surface realization will be spatially and temporally correlated. However, if we are looking at statistics of many such sets in a Monte Carlo fashion, we can treat different paths on the surface as being independent. A specific instance where this is useful is looking at statistics for propagating radio waves in a multistatic environment as shown in Figure \ref{os_fig:16} where there are 3 separate paths for propagation.
...
\lstinputlisting[language=Matlab,caption=Elfouhaily 1-D Spectrum Matlab Code,label=list:elf1d]{../mfiles/SeaSurfaceGeneration/Elfouhaily.m}

\lstinputlisting[language=Matlab,caption=1-D Ocean Surface Generation Matlab Code,label=list:surf1d]{../mfiles/SeaSurfaceGeneration/generateSeaSurface.m}

Listing \ref{list:elf2d} computes the 2-D Elfouhaily Spectrum and Listing \ref{list:surf2d} uses this spectrum to generate a 2-D random surface.

\lstinputlisting[language=Matlab,caption=Elfouhaily 2-D Spectrum Matlab Code,label=list:elf2d]{../mfiles/SeaSurfaceGeneration/Elfouhaily2D.m}

\lstinputlisting[language=Matlab,caption=2-D Ocean Surface Generation Matlab Code,label=list:surf2d]{../mfiles/SeaSurfaceGeneration/generateSeaSurface2D.m}